...
enum AIType { WALKER, GUARD, FLYER, CHASER, ZIGZAG };
...
private:
    ...
    glm::vec3 m_movement;
    glm::vec3 m_position;
	glm::vec3 m_starting_position; // For Walker AI
    glm::vec3 m_scale;
    glm::vec3 m_velocity;
    ...
    float m_fly_time = 0.0f;

    float m_width = 1.0f,
        m_height = 1.0f;

    bool m_is_ship = false;

    GLuint m_booster_texture_id = 0;
    int m_booster_frame = 0;
    float m_booster_animation_time = 0.0f;
    int m_booster_cols = 2; // Since it’s a 2-column sprite
...
public:
    // ————— STATIC VARIABLES ————— //
    static constexpr int SECONDS_PER_FRAME = 20;
    ...
    // Collision logic
    bool check_collision_above(Entity* enemy) const;
    ...
    void const set_position(glm::vec3 new_position) { m_position = new_position; m_starting_position = new_position; }
    ...
    void const set_animation_indices(int* indices) { m_animation_indices = indices; }
	...
	void set_is_ship(bool is_ship) { m_is_ship = is_ship; }
    void set_lives(int lives) { m_lives = lives; }
    void set_booster_texture_id(GLuint id) { m_booster_texture_id = id; }
};
