...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Space Blaster",
...
void process_input()
{
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_SPACE:
                if (g_current_scene != g_menu)
                {
                    ...
                    Entity* beam = state.beam;
                    Entity* player = state.player;

                    if (!beam->is_active())
                    {
                        beam->activate();
                        beam->set_position(glm::vec3(player->get_position().x, player->get_position().y + 0.8f, 0.0f)); // offset slightly above
                        beam->set_movement(glm::vec3(0.0f, 1.0f, 0.0f)); // fire upward
                    }
    ...
    if (g_current_scene->get_state().player != nullptr) // Only accept input if player exists
    {
        ...
        if (key_state[SDL_SCANCODE_W]) movement.y = 1.0f;
        if (key_state[SDL_SCANCODE_S]) movement.y = -1.0f;
        if (key_state[SDL_SCANCODE_A]) movement.x = -1.0f;
        if (key_state[SDL_SCANCODE_D]) movement.x = 1.0f;

        g_current_scene->get_state().player->set_movement(movement);
...
void update()
{
	...
	if (g_current_scene->get_state().player != nullptr) // Only move camera if player exists
	{
